...
def main():
    ...
    # Load our data.
    input_dataframe = sql_context.read.option("header", "true").csv(INPUT_PATH1)
    input_dataframe = sql_context.read.option("header", "true").csv(INPUT_PATH2)

    # Save the results as Parquet.
    input_dataframe.write.mode("overwrite").parquet(OUTPUT_PATH1)
    input_dataframe.write.mode("overwrite").parquet(OUTPUT_PATH2)
